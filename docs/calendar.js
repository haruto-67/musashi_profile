(function(){
  var W=["日","月","火","水","木","金","土"];
  var host=document.getElementById("cal"); if(!host) return;
  ...
  var body=document.createElement("div"); body.className="calbody";
  host.appendChild(nav); host.appendChild(body);

  function listOf(d){
    var key=d.getFullYear()+"-"+("0"+(d.getMonth()+1)).slice(-2);
    return GIGS.filter(function(g){return g.d.indexOf(key)===0;});
  }
  months.forEach(function(d,i){
    var b=document.createElement("button");
    b.type="button"; b.className="mbtn";
  ...
  function draw(i){
    ...
    var d0=months[i], y=d0.getFullYear(), mo=d0.getMonth();
    var key=y+"-"+("0"+(mo+1)).slice(-2), list=listOf(d0);
    var last=new Date(y,mo+1,0).getDate(), pad=d0.getDay();
    var h='<p class="calcap">'+y+'年'+(mo+1)+'月</p>';
    h+='<div class="dow">'+W.map(function(w){return "<span>"+w+"</span>";}).join("")+'</div>';
    h+='<div class="days">';
